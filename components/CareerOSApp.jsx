...
async function getJobs(profile, prefs) {
  ...
  // Apply-URL guard is absolute (no fake links). Location is applied as a RELAXING
  // cascade so we never dead-end on "no jobs" when real US jobs exist.
  const withLink = raw.filter(j => { const link = j.applyUrl || j.url; return link && /^https?:\/\//i.test(link); });
  let pool = withLink.filter(j => passesLocation(j, prefs));        // strict: location gate
  let tier = "exact";
  if (pool.length < 3) {                                            // relax to any US / remote
    const usPool = withLink.filter(j => looksUS(j.location) || (j.location||"").toLowerCase().includes("remote") || j.workType === "Remote");
    if (usPool.length > pool.length) { pool = usPool; tier = "similar"; }
  }
  if (pool.length === 0) { pool = withLink; tier = "stretch"; }      // last resort: anything with a real link
  debug.returned = pool.length; debug.tier = tier;
  if (pool.length === 0) { debug.error = "Connected, but no live roles came back for these filters right now."; return { jobs: [], errors, source: "proxy", debug }; }
  // rank by location relevance, take top 10
  let scoped = pool.slice().sort((a, b) => locationTier(a, prefs) - locationTier(b, prefs)).slice(0, 10);
  ...
  console.log("[CareerOS] jobs reaching UI:", jobs.length, "tier:", tier);
...
export default function CareerOS() {
  ...
  const refreshJobs=async(override)=>{setJobsLoading(true);const p={...prefs,...(override||{})};if(override)setPrefs(p);
    setJobs([]); setJobErrors([]); setJobDebug(null);   // clear stale results first (#6)
