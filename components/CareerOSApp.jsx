...
import { scoreResumeLocal, estimateSalaryLocal, matchJobLocal } from "../lib/localEngine";
...
function recommendTitlesLocal(profile) {
  ...
  const out = [...new Set(roles)].slice(0, 3);
...
export default function CareerOS() {
  ...
  const [buildMsg,setBuildMsg]=useState(""); const [parsing,setParsing]=useState(false); const [reupload,setReupload]=useState(false); const [enhancing,setEnhancing]=useState(false);
  ...
  const onFile=async e=>{const f=e.target.files?.[0];if(!f)return;setParsing(true);setError("");
    try{
      ...
      const m=err?.message;
      setError(m==="EMPTY"?"We couldn't read text from that file — it may be a scanned image. Try a text-based PDF/DOCX, or build manually.":m==="UNSUPPORTED"?"Use a PDF, DOCX, or TXT file — or build manually.":"Couldn't read that file. Try another, or build manually.");
      setParsing(false);
    }};
  ...
  if(flow!=="app"){
    return (<div className="cos"><style>{STYLES}</style>
      {flow==="welcome"&&(<div className="cos-onb" style={{justifyContent:"center",textAlign:"center"}}>
        ...
        <button className="cos-btn cos-btn-ghost cos-btn-block" style={{marginTop:10}} disabled={parsing} onClick={()=>{setProfile(empty());setFlow("wizard");}}><Pencil size={15}/> Build Manually</button>
...
function Onboarding({ profile, setProfile, prefs, setPrefs, customSal, setCustomSal, recommended, enhancing, onExit, onFinish }){
  ...
  return (<div className="cos-wiz">
    ...
    <div className="cos-wiz-body">
      {step===0&&(<div className="cos-step" key="s0">
        ...
        <div className="cos-field"><span className="cos-label">Full name</span><input className="cos-input" value={profile.name||""} onChange={e=>setP("name",e.target.value)} placeholder="Jordan Rivera"/></div>
        <div className="cos-field"><span className="cos-label">Email</span><input className="cos-input" value={profile.email||""} onChange={e=>setP("email",e.target.value)} placeholder="you@email.com"/></div>
        <div className="cos-field"><span className="cos-label">Phone</span><input className="cos-input" value={profile.phone||""} onChange={e=>setP("phone",e.target.value)} placeholder="(555) 555-5555"/></div>
