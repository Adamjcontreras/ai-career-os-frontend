...
import { APP_VERSION, COMMIT_SHORT } from "../lib/version";
...
const EXTRACT_PROMPT = `You are a resume parser. Read the ENTIRE resume and extract EVERYTHING accurately. Capture every job (with employer, dates, and ALL bullet points/accomplishments verbatim), every certification, all education, and all skills. Do not summarize or drop content. Return ONLY valid JSON, no markdown fences:
{"name":"","email":"","phone":"","city":"","state":"","summary":"2-3 sentence professional summary based on the resume","skills":[],"technicalSkills":[],"certifications":[{"name":"","issuer":"","earned":"","expires":""}],"workHistory":[{"title":"exact job title","company":"employer","dates":"e.g. 2020 - Present","highlights":["each bullet/accomplishment verbatim"]}],"education":[{"degree":"","school":"","year":""}],"projects":[],"leadership":[],"clearance":"None|Public Trust|Secret|Top Secret|TS/SCI|Other","military":false,"branch":"","desiredRoles":["5-7 realistic civilian job titles this person should target based on their experience; translate military roles to civilian equivalents"]}`;
...
async function extractProfile(file) {
  ...
  let text = "";
  // Always get the raw text too (used by the optimizer as the source of truth).
  try { text = await extractText(file); } catch (e) { if (e.message==="UNSUPPORTED") throw e; }

  // PRIMARY: send the FULL resume to the AI and let it read the whole thing
  // (this is the old behavior that produced complete, accurate profiles).
  try {
    let content;
    ...
    else { if (!text.trim()) throw new Error("EMPTY"); content = EXTRACT_PROMPT + "\n\nResume text:\n" + text; }
    const ai = parseJSON(await callClaude([{ role:"user", content }], { maxTokens: 2200, timeoutMs: 35000 }));
    if (ai && (ai.name || (ai.workHistory||[]).length || (ai.skills||[]).length)) {
      return { profile: { ...ai, rawResume: text, _source: "ai" }, text };
    }
  } catch (e) { /* fall through to local */ }

  // FALLBACK: local heuristic parser (no AI). Keeps the app usable without credits.
  if (!text) text = await extractText(file);
  ...
  return { profile: { ...local, rawResume: text }, text };
}
...
async function fromProxy(profile, prefs) {
  ...
  if (profile?.clearance && !/^none$/i.test(profile.clearance)) u.searchParams.set("clearance", profile.clearance);
  else if (prefs?.clearancePref && !["None","Any"].includes(prefs.clearancePref)) u.searchParams.set("clearance", prefs.clearancePref);
...
async function getJobs(profile, prefs) {
  ...
  const withLink = raw.filter(j => { const link = j.applyUrl || j.url; return link && /^https?:\/\//i.test(link); });
  // Tag every job: international if it looks foreign and not US.
  withLink.forEach(j => {
    const loc = (j.location||"").toLowerCase();
    j._intl = FOREIGN_HINTS.some(h => loc.includes(h)) && !looksUS(j.location);
  });
  const domestic = withLink.filter(j => !j._intl);
  const intl = withLink.filter(j => j._intl);

  // DOMESTIC pool relaxes location WITHIN the US only — it never substitutes foreign jobs.
  let pool = domestic.filter(j => passesLocation(j, prefs));
  let tier = "exact";
  if (pool.length < 3) {
    const usWide = domestic.filter(j => looksUS(j.location) || (j.location||"").toLowerCase().includes("remote") || j.workType === "Remote");
    if (usWide.length > pool.length) { pool = usWide; tier = "similar"; }
  }
  if (pool.length === 0 && domestic.length) { pool = domestic; tier = "stretch"; }
  debug.returned = pool.length; debug.tier = tier; debug.intlCount = intl.length;
  if (pool.length === 0 && intl.length === 0) { debug.error = "Connected, but no live roles came back for these filters right now."; return { jobs: [], intlJobs: [], errors, source: "proxy", debug }; }
  // rank domestic by location relevance
  let scoped = pool.slice().sort((a, b) => locationTier(a, prefs) - locationTier(b, prefs)).slice(0, 12);
  ...
  // international jobs scored locally (kept in their own list for the Intl tab)
  const intlJobs = intl.slice(0, 12).map(j => matchJobLocal(profile, prefs, j)).sort((a,b)=>(b.match||0)-(a.match||0));
  console.log("[CareerOS] domestic:", jobs.length, "intl:", intlJobs.length, "tier:", tier);
  debug.returned = jobs.length;
  return { jobs, intlJobs, errors, source: "proxy", debug };
}
...
function fallbackOpt(p){ const sk=(p.skills||[]).concat(p.technicalSkills||[]);
  // If we have the original resume text, KEEP it as the base (don't strip it down).
  const full = (p.rawResume && p.rawResume.length>120) ? p.rawResume : localResumeText(p);
...
async function optimize(profile, job, onStage){ onStage("Analyzing resume…"); await new Promise(r=>setTimeout(r,300)); onStage("Rewriting bullet points…");
  const jc=job?`\nTailor to ${job.title} at ${job.company}. Weave in relevant keywords: ${(job.missing||[]).join(", ")}.`:"";
  const source = profile.rawResume && profile.rawResume.length>120
    ? `ORIGINAL RESUME (preserve ALL roles, employers, dates, and every accomplishment — improve wording, do not drop anything):\n${profile.rawResume.slice(0,9000)}`
    : `Candidate data:\n${JSON.stringify(profile)}`;
  const prompt=`Rewrite this resume to be stronger, truthful, and ATS-friendly. KEEP the full resume — every job, employer, date, and bullet must remain (improved, never removed). Expand weak bullets with strong action verbs and measurable impact. Keep the candidate's real content as the source of truth.${jc}
Output PLAIN TEXT with EXACTLY these markers, no markdown:
${D.s}\n<2-3 sentence professional summary>\n${D.b}\n<one improved bullet per line, 5-8 of the strongest>\n${D.k}\n<one ATS keyword per line>\n${D.sk}\n<one skills tip per line>\n${D.r}\n<the COMPLETE optimized resume in plain text: header, summary, all skills, ALL experience with employers/dates/bullets, certifications, education>\n${D.e}
${source}`;
  let parsed=null;
  try { const out=await callClaude([{role:"user",content:prompt}],{maxTokens:3000, timeoutMs:35000}); onStage("Generating optimized resume…"); parsed=parseOpt(out); }
...
export default function CareerOS() {
  ...
  const empty=()=>({name:"",email:"",phone:"",city:"",state:"",summary:"",skills:[],technicalSkills:[],certifications:[],workHistory:[],education:[],projects:[],leadership:[],clearance:"",military:false,branch:"",rawResume:"",desiredRoles:[]});
  ...
  const [jobs,setJobs]=useState([]); const [intlJobs,setIntlJobs]=useState([]); const [jobScope,setJobScope]=useState("domestic"); const [jobErrors,setJobErrors]=useState([]); const [jobDebug,setJobDebug]=useState(null);
  ...
  const onFile=async e=>{const f=e.target.files?.[0];if(!f)return;setParsing(true);setError("");
    activity.mark("upload", f.name);
    try{
      const { profile:parsed }=await extractProfile(f);   // AI reads the FULL resume (local fallback)
      const np={...empty(),...parsed};
      activity.mark("parse", `${np.name||"?"}, ${(np.workHistory||[]).length} roles, ${(np.skills||[]).length} skills`);
      setProfile(np);
      setFromResume(true);
      // Seed prefs from the resume: location + desired roles, so the user just reviews.
      setPrefs(pr=>({ ...pr,
        city: np.city||pr.city, state: np.state||pr.state,
        titles: (np.desiredRoles&&np.desiredRoles.length)?np.desiredRoles.slice(0,4):(np.targetRoles||pr.titles),
      }));
      const recs = (np.desiredRoles&&np.desiredRoles.length)?np.desiredRoles:(np.targetRoles||[]);
      if(recs.length) setRecommended(recs);
      setFlow("wizard");
      setParsing(false);
    }catch(err){
      const m=err?.message;
  ...
  const runAnalysis=async(p,pr,{force=false}={})=>{
    ...
    getJobs(p,pr).then(jr=>{setJobs(jr.jobs);setIntlJobs(jr.intlJobs||[]);setJobErrors(jr.errors);setJobDebug(jr.debug);
      activity.mark("match", `${jr.jobs.length} jobs, tier ${jr.debug?.tier||"?"}`);
      coachInsights(p,jr.jobs,null,pr).then(ins=>{setInsights(ins);activity.mark("coach","insights");}).catch(()=>{}).finally(()=>setLoading(l=>({...l,coach:false})));
  ...
  const refreshJobs=async(override)=>{setJobsLoading(true);const p={...prefs,...(override||{})};if(override)setPrefs(p);
    setJobs([]); setIntlJobs([]); setJobErrors([]); setJobDebug(null);   // clear stale results first (#6)
    setLoading(l=>({...l,jobs:true}));
    try{const jr=await getJobs(profile,p);setJobs(jr.jobs);setIntlJobs(jr.intlJobs||[]);setJobErrors(jr.errors);setJobDebug(jr.debug);}
  ...
  const onReupload=async e=>{const f=e.target.files?.[0];if(!f)return;setReupload(true);
    try{const { profile:parsed }=await extractProfile(f);const np={...empty(),...parsed};setProfile(np);setPrevScore(score?.score??null);
      analyzedSig.current=null; // invalidate cache so everything recomputes
      const recs=(np.desiredRoles&&np.desiredRoles.length)?np.desiredRoles:(np.targetRoles||[]); if(recs.length)setRecommended(recs);
  ...
  const activeJobs = jobScope==="intl" ? intlJobs : jobs;
  const buckets={strong:activeJobs.filter(j=>(j.match||0)>=75),good:activeJobs.filter(j=>(j.match||0)>=60&&(j.match||0)<75),stretch:activeJobs.filter(j=>(j.match||0)<60)};
  ...
  return (<div className="cos"><style>{STYLES}</style>
    ...
    <div className="cos-scroll" ref={scrollRef} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
      ...
      {tab==="jobs"&&(<div className="cos-reveal">
        ...
        {/* Domestic / International tabs — foreign jobs live ONLY under International */}
        <div style={{display:"flex",gap:8,marginBottom:12,background:"var(--paper)",padding:4,borderRadius:12}}>
          <button onClick={()=>setJobScope("domestic")} style={{flex:1,padding:"9px",borderRadius:9,border:"none",cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:"inherit",background:jobScope==="domestic"?"var(--card)":"transparent",color:jobScope==="domestic"?"var(--primary)":"var(--muted)",boxShadow:jobScope==="domestic"?"0 1px 3px rgba(0,0,0,.06)":"none"}}>🇺🇸 Domestic{jobs.length?` (${jobs.length})`:""}</button>
          <button onClick={()=>setJobScope("intl")} style={{flex:1,padding:"9px",borderRadius:9,border:"none",cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:"inherit",background:jobScope==="intl"?"var(--card)":"transparent",color:jobScope==="intl"?"var(--primary)":"var(--muted)",boxShadow:jobScope==="intl"?"0 1px 3px rgba(0,0,0,.06)":"none"}}>🌍 International{intlJobs.length?` (${intlJobs.length})`:""}</button>
        </div>
        ...
        :activeJobs.length===0?(<div className="cos-card cos-empty"><div className="cos-empty-ic"><Search size={24}/></div>
          <div style={{fontWeight:700,fontSize:16}}>{jobScope==="intl"?"No international jobs":"No jobs found yet"}</div>
          <p style={{color:"var(--muted)",fontSize:13.5,margin:"7px auto 0",lineHeight:1.45,maxWidth:300}}>{jobScope==="intl"?"This search didn't return roles outside the U.S. Try a broader title or Remote.":"Try widening your search or switching to remote."}</p>
          ...
          {DEBUG&&<div style={{textAlign:"left",fontSize:11.5,color:"var(--muted)",marginBottom:8,fontFamily:"ui-monospace,monospace"}}>🛠 {jobDebug?.error||(JOB_PROXY_URL?"Connected · no results for these filters.":"Dev: JOB_PROXY_URL not set.")}{jobErrors&&jobErrors.length?` · sources failed: ${jobErrors.join(", ")}`:""}</div>}
          {jobErrors&&jobErrors.length>0&&(<div style={{textAlign:"left",background:"#FBF3DF",color:"#7A5912",borderRadius:10,padding:"9px 12px",fontSize:12,marginBottom:10,lineHeight:1.45}}>Some job sources didn't respond ({jobErrors.join(", ")}). This usually means an API key isn't set on the backend. Other sources still work.</div>)}
    ...
    <div style={{fontSize:9.5,color:"var(--muted)",textAlign:"center",padding:"2px 0",opacity:.6}}>{APP_VERSION} · {COMMIT_SHORT} · <a href="/diagnostics" style={{color:"inherit"}}>diagnostics</a></div>
    ...
    {filterSheet&&(<div className="cos-sheet modal"><div className="cos-sheet-grab"/>
      ...
      <div className="cos-sheet-body">
        ...
        <span className="cos-label">Clearance</span><div className="cos-chips" style={{marginBottom:12}}>{["Any","None","Public Trust","Secret","Top Secret","TS/SCI","Polygraph"].map(x=><button key={x} className={`cos-chip ${prefs.clearancePref===x?"on":""}`} onClick={()=>setPrefs({...prefs,clearancePref:x})}>{x}</button>)}</div>
  ...
  return (<div className="cos-wiz">
    ...
    <div className="cos-wiz-body">
      ...
      {step===1&&(<div className="cos-step" key="s1">
        ...
        {(()=>{const est=estimateSalaryLocal(profile,prefs);const range=`${est.expectedLow}–${est.expectedHigh}`;return(
          <div className="cos-banner ok" style={{marginBottom:10,cursor:"pointer"}} onClick={()=>{setPrefs(p=>({...p,salaryTarget:range}));setCustomSal(range);}}>
            <Lightbulb size={15} style={{flexShrink:0}}/>We estimate your market value at <b style={{margin:"0 3px"}}>{range}</b>. Tap to use this range.</div>);})()}
