...
async function fromProxy(profile, prefs) {
  ...
  const r = await fetch(u.toString()); if (!r.ok) throw new Error("proxy");
  const d = await r.json();
  return (d.jobs||[]).map(j => ({ ...j, applyUrl: j.applyUrl || j.url }));
}
async function sourceRemotive(role) {
  const r = await fetch(`https://remotive.com/api/remote-jobs?limit=40${role?`&search=${encodeURIComponent(role)}`:""}`); if (!r.ok) throw 0;
  const d = await r.json();
  return (d.jobs||[]).map(j => ({ title:j.title, company:j.company_name, location:j.candidate_required_location||"Remote", workType:"Remote",
    salary:j.salary||"", description:stripHtml(j.description).slice(0,400), source:"Remotive", posted:(j.publication_date||"").slice(0,10), url:j.url, applyUrl:j.url }));
}
async function sourceArbeitnow() {
  const r = await fetch("https://www.arbeitnow.com/api/job-board-api"); if (!r.ok) throw 0;
  const d = await r.json();
  return (d.data||[]).slice(0,40).map(j => ({ title:j.title, company:j.company_name, location:j.location||(j.remote?"Remote":""), workType:j.remote?"Remote":"On-Site",
    salary:"", description:stripHtml(j.description).slice(0,400), source:"Arbeitnow", posted:j.created_at?new Date(j.created_at*1000).toISOString().slice(0,10):"", url:j.url, applyUrl:j.url }));
}
...
// Returns { jobs, errors, source, debug }. NO sample fallback — empty means empty.
async function getJobs(profile, prefs) {
  // Build the search title set from selected roles + resume-implied roles
  ...
  const errors = []; let raw = []; let usedProxy = false;
  const debug = { connected: !!JOB_PROXY_URL, source: JOB_PROXY_URL ? "proxy" : "direct (keyless)",
  ...
  if (JOB_PROXY_URL) {
    try { raw = await fromProxy({ ...profile, _titles: searchTitles }, { ...prefs, titles: searchTitles }); usedProxy = true; }
    catch (e) { errors.push("Job service"); debug.error = "Proxy unreachable: " + (e?.message || "error"); }
  }
  if (!usedProxy) {
    // search each title separately (Remotive supports search); Arbeitnow is a single board
    const tasks = [...searchTitles.map(t => ["Remotive:" + t, () => sourceRemotive(t)]), ["Arbeitnow", () => sourceArbeitnow()]];
    const results = await Promise.allSettled(tasks.map(([, fn]) => fn()));
    results.forEach((r, i) => { if (r.status === "fulfilled") raw.push(...r.value); else errors.push(tasks[i][0].split(":")[0]); });
    if (raw.length === 0 && errors.length) debug.error = "Direct sources returned nothing from this browser (" + [...new Set(errors)].join(", ") + "). Connect a proxy for live jobs.";
  }
  // dedupe
  ...
  if (raw.length === 0) {
    if (!debug.error) debug.error = JOB_PROXY_URL ? "API connected but returned no results for these filters." : "No job service connected (JOB_PROXY_URL is empty).";
    return { jobs: [], errors, source: usedProxy ? "proxy" : "direct", debug };
  }
  // cascade location relevance; never drop to empty
