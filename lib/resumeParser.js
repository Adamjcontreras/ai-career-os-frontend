...
export function parseResumeText(text) {
  ...
  // Work history: a real role line has a date RANGE (2019–2022 / 2020–Present)
  // OR a title-like phrase next to a year. Reject contact/location/skills/edu lines.
  const workHistory = [];
  ...
  for (let i = 0; i < lines.length && workHistory.length < 6; i++) {
    const l = lines[i];
    const hasRange = rangeRe.test(l);
    const looksTitle = isTitleLike(l.replace(yearRe, "").replace(/[-–—|,].*$/, "").trim());
    if (!hasRange && !(yearRe.test(l) && looksTitle)) continue;
    if (degRe.test(l)) continue;                       // education line
    if (phoneLike.test(l) || l.includes("@")) continue; // contact line
    if (new RegExp(`,\\s*(${STATE_ABBR})\\b`).test(l) && !looksTitle) continue; // location line
    const dates = (l.match(rangeRe) || l.match(yearRe) || [""])[0];
    let title = "", company = "";
    const sep = l.split(/\s+(?:at|@|—|–|\||,)\s+/);
    if (sep.length >= 2) { title = clean(sep[0].replace(rangeRe, "").replace(yearRe, "")); company = clean(sep[1].replace(rangeRe, "").replace(yearRe, "")); }
    else { title = clean(l.replace(rangeRe, "").replace(yearRe, "")); }
    title = title.replace(/[•|].*$/, "").trim().slice(0, 80);
    if (!title || title.length < 3) continue;
    // collect up to 4 following bullet lines as highlights
    const highlights = [];
    for (let k = i + 1; k < lines.length && highlights.length < 4; k++) {
      const b = lines[k];
      if (rangeRe.test(b) || degRe.test(b)) break;       // next role/section
      if (/^[-•*\u2022]/.test(b) || (b.length > 25 && /\b(led|managed|built|developed|supervised|maintained|designed|deployed|coordinated|improved|reduced|increased|trained|operated|delivered)\b/i.test(b))) {
        highlights.push(b.replace(/^[-•*\u2022\s]+/, "").trim());
      }
    }
    workHistory.push({ title, company: company.slice(0, 80), dates: clean(dates), highlights });
  }
