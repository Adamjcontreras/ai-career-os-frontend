...
export const applications = {
  list: async () => (await db.from("applications").select("*").order("created_at", { ascending: false })).data || [],
  ...
  setStage: (id, stage) => db.from("applications").update({
...
export const savedMatches = {
  list: async () => (await db.from("saved_matches").select("*").order("created_at", { ascending: false })).data || [],
  async add(job) {
    const { data: u } = await db.auth.getUser();
    return db.from("saved_matches").insert({ user_id: u.user.id, job_payload: job, match_score: job.match });
  },
  remove: (id) => db.from("saved_matches").delete().eq("id", id),
};

/* ---------------- coach history ---------------- */
export const coach = {
  history: async () => (await db.from("coach_messages").select("*").order("created_at", { ascending: true })).data || [],
